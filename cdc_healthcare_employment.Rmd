...
employment_wages <- 
  ...
  mutate(employment_delta = employment - lag(employment),
         mean_hourly_wage_delta = mean_hourly_wage - lag(mean_hourly_wage), 
         employment_pct_chg = round(((employment - lag(employment)) / lag(employment)), 3),
         mean_hourly_wage_pct_chg = round(((mean_hourly_wage - lag(mean_hourly_wage)) / lag(mean_hourly_wage)), 4)) |> 
  arrange(occupation_title)
...
``` {r graph}
employment_wages |> 
  ggplot(aes(x = occupation_title, y = employment_pct_chg)) +
  geom_boxplot()
