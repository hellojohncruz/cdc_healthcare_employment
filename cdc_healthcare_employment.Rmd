...
employment_wages <- 
  ...
  mutate(emp_delta = employment - lag(employment),
         ...
         emp_pct_chg = round(((employment - lag(employment)) / lag(employment)), 3),
...
Lets find the top 10 percentage change in wages based on occupation

``` {r top-10}
top_10 <- 
  employment_wages |> 
  arrange(desc(wage_pct_chg)) |> 
  select(occupation, wage_pct_chg) |> 
  head(10)
...
With the top 10 occupations, lets look at each of their historical wage trends compared to employment percentage changes.
...
top_10_history <- 
  employment_wages |> 
  filter(occupation %in% top_occ_names)
```

``` {r graph}
top_10_history |>
  ggplot(aes(x = year, y = wage_pct_chg)) +
